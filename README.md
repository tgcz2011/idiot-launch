...
3. **多镜像源 fallback（7 个源）**：下载依次尝试 GitHub 直连 → gh-proxy.com → ghfast.top → ghproxy.net → gh.llkk.cc → hub.gitmirror.com → ghproxy.homeboyc.cn，适配校园不稳定网络。
4. **动态超时**：每个源基础超时 120 秒，第 1 轮 1x（120s）、第 2 轮 2x（240s）、第 3 轮 3x（360s）——避免所有源都在短超时内失败后永远更新不了，最多重试 3 轮。
5. **SHA-256 校验**：Release body 附带安装包哈希，下载完成后校验，对不上则重新下载并拒绝更新。
6. **倒计时退出时静默更新**：下载完成后等待 Countdown Desktop 退出，退出后删除旧目录并静默安装新版。
7. **启动时补装**：若倒计时一直开着，下次启动时若已下载且未运行则立即补装。
8. **状态持久化**：`D:\IdiotLaunch\data\state.json` 记录所有状态，冰点还原不影响。
9. **统一管理更新**：Idiot Launch 启动 Countdown Desktop 时传入 `--auto-check-update off`，由 Idiot Launch 统一管理更新，避免重复检查。
