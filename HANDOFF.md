...
- Countdown Desktop v3.2.0.0 内置单实例接管：已有实例运行时，新启动自动通知旧实例退出并接管，后启动者覆盖先启动者效果。
...
├── installer/
│   └── CountdownDesktop_Setup_3.2.0.0.exe  （构建时下载，gitignore）
├── tools/                      辅助脚本（预留）
...
2. **安装包版本固定**：当前内嵌 v3.2.0.0，升级 Countdown Desktop 版本需更新 `INSTALLER_REL`、`build.ps1`、`release.yml` 和 README 中的版本号。
...
# 3. 验证静默安装（手动测试）
.\installer\CountdownDesktop_Setup_3.2.0.0.exe /VERYSILENT /NORESTART /SUPPRESSMSGBOXES /DIR=D:\CountdownDesktop
